...
- *Since* `atanas` *was* ***a member of the privileged*** `disk` ***group,*** we went into **a rabbit hole** trying to exploit that using the `debugfs` tool.
- *Not going anywhere,* we searched the file system again to **find a file owned by our user** called `app.log` ***within the root folder.***
- The log file contained past events of **another server using a vulnerable version** of `wget` to fetch data from ours.
- *After finding a tool called* `authbind` (it lets us use low ports without being root) *installed on the system,* we use it to **deploy a python web server** and verify that ***there were indeed requests still being made to our machine.***
