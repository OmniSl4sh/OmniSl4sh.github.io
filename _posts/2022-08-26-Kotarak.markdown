...
*However, just to be sure,* we're going to try it with both `root` and `atanas`. since we didn't find other users locally with a proper shell.
...
- that file would then be **saved to the victim's home folder** becoming `wget`'s main configuration. since it would search for it there first.
...
The exploit now **served the** `cron` **containing our reverse shell** and **it was downloaded by the victim** to their `/etc/cron.d`
