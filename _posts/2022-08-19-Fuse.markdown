...
- On that web page, we find the printing activity of some domain users. That gets us an initial list of usernames, workstations and document names.
- One of the documents' name stuck out: `"Fabricorp01.docx"` which looked like a possible password/password scheme to try.
- After spraying that password and others with the same pattern, We find that it was indeed used by 3 of the users found on the web page. But was expired and had to be changed.
- Using the linux `smbpassswd` utility, we were able to change passwords and gain access. But we didn't get much further with traditional SMB share enumeration and AD Attacks (ASREPRoasting, Kerberoasting etc.).
- However, when inspecting `Bloodhound`'s output, we notice that there's a potential path to Domain Administrator if we compromise the `svc-print` account. That's because it has PowerShell Remoting access and is a member of the Print Operators AD group which hold the dangerous `SeLoadDriver` privilege.
- Following the theme of printing which was recurring throughout the machine, we enumerate the printers of the domain to find a password in one of the printers' description field.
...
- We abuse its `SeLoadDriverPrivilege` to load a vulnerable driver into the kernel and exploit that to get code execution as `NT Authority\System`.
...
we also know that WSMan port 5985 is open which is good if we get a user with PowerShell Remoting access.
...
### RPC Enumeration
My favourite RPC enumeration tool [`enum4linux-ng`](https://github.com/cddmp/enum4linux-ng) doesn't yield much more information either. I usually take away the Domain SID. But that's it.
```bash
...
### LDAP Enumeration
And the same goes for LDAP: anonymous bind isn't enabled:

![](/assets/Fuse/no-ldap-anon.jpg)

### The Website
Having cleared away all the quick and simple checks, it's time to take a look at the website.
...
Checking the first one, we take note of the usernames as well as the document and client names.
...
A gut feeling tells us: "That's a password" XD

And if not, it wouldn't be exhausting to try `Fabricorp02`, `Fabricorp03` and so on...
...
After failing to reset the password dusing RPC's `setuserinfo2` command, we do some further research to find a handy tool called `smbpasswd`.

It was built for administering Samba but Let's take a look at its documentation:

![](/assets/Fuse/smbpasswd-docs.jpg)

Jackpot! This is exactly what we need. let's change the password for a user and see if it works.
...
3. Doing a Bloodhound collection to see what's Kerberoastable, what's ASREPRoastable, what privileges our users have etc.
...
As explained in the image above, after we succesfully reset one user's password, it changed back to what it was.
...
To make our lives easier, we'll have to counter that automation with one of our own. Let's look at the help for the `smbpasswd` utility to see what options it offers:

![](/assets/Fuse/smbpasswd-help.jpg)

With the help of the `-s` flag, we can pass in passwords through `stdin`.

the old password + the new one + the confirmation would be fed to `smbpasswd` with a new line in between.

minding that a new password must be given every time we do a reset. Because AD -by default- remembers the last 24 passwords you used on a given account.
...
We modify the script to target only one user and pull all the domain users using `crackmapexec`

![](/assets/Fuse/getting-all-ad-users.jpg)

We spray all the remaining ones with `Fabricorp01` through `Fabricorp09` but with no success :/

![](/assets/Fuse/2nd-password-spray.jpg)

### AD Enumeration with Bloodhound 
After pulling a BloodHound collection using [`Bloodhound.py`](https://github.com/fox-it/BloodHound.py), We don't find anything exploitable with any of our owned users.
...
This account is our target if we want to take over the domain.

Because it can:
1. Gain a remote shell into the Domain Controller (since it's part of the "Remote Management Users" group)
...
### Printer Enumeration
We can enumerate printers through RPC with a valid account.

Let's re-run `enum4linux-ng` with credentials this time and see what comes up.

![](/assets/Fuse/rpc-printer-enumeration.jpg)

A password is there in the description :D

Naturally, we spray it to find both `svc-print` and `svc-scan` using it.
...
### Evil Driver Pwnage
The amazing article by Tarlogic Cybersecurity [here](https://www.tarlogic.com/blog/abusing-seloaddriverprivilege-for-privilege-escalation/#0x01_-_Preamble) explains the theory and details the steps of exploitation. Give it a read for the full picture.
...
We compile the `EoPLoadDriver.exe` using Visual Studio
...
And generate the reverse shell executable using `msfvenom`
...
Now locked and loaded, we move the `rev.exe` back one directory to be in `C:\Users\svc-print\Documents` matching the path specified in the `ExploitCapcom.exe` executable

And proceed to load `Capcom.sys` into the writable `HKCU:\System\CurrentControlSet` registry path using `EoPLoadDriver.exe` and name it `KillerDriver` for dramatic effect XD
