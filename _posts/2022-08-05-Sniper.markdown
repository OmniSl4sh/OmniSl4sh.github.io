...
### Summary
- The box is a **Windows** machine **hosting a PHP website** which had both a **LFI** (intended) and **RFI** (unintended) vulnerabilities.
...
- *Also relevant to the same topic,* a folder called `Docs` in the root of the `C:` drive.
- The folder contained a **note for the developer** (`chris`) from the CEO **asking for the documentation to be dropped there** when done.
...
- *After placing the file in the `Docs` folder*, a few moments pass and the file gets removed. We verify execution of our payload (resetting the `administrator`'s password) by logging in and we're authenticated successfully.
...
Nmap gives a few things to check:
1. Port 80
2. **RPC** on port 135
3. **SMB** on port 445
...
This use has a special type of security. No one could ever guess creds like these :D :D :D

*Jokes aside,* here are the **requests and responses for the registration and login**.

the response we got was a **302 redirection to the initial login page**
...
1. **Not accepting** any `..` sequences
2. **Not accepting** `c:` in the request
3. **Doesn't like the path** when it's missing a `/` or `\` in the beggining
4. **But is alright with specifying the path without the** `c:` **part**
...
- `<` and `>` are the **tags** needed to **inject PHP**
...
- *also,* the **dot** (`.`) is blocked, so we can't use our **ip address** in the payload to download something by either HTTP or SMB
...
*Even though the dash* (`-`) *is blocked*, this trick is still accessible with the *whitelisted* **forward slash** (`/`).
...
*thankfully, after all this hard work,* we're rewarded with a **sweeet reverse shell :D**

![shell-as-iusr](/assets/Sniper/shell-as-iusr.jpg)

### Finding creds in the database settings file (like always)
*The first I did after getting the shell,* was **check the files in the web root.**

I find a file called `db.php` in the `C:\inetpub\wwwroot\user` directory.
...
The situation is that we want to pivot to the `chris` user **without losing the shell** (*that we worked so hard for XD*)

We can do so from our **PowerShell** reverse shell using the below method:
...
*other than that, according to the text,* it seems like the chris guy is really stressed out and thinking about quitting the his job because the CEO is overloading him with work.

We don't know if this information would be handy or not. But we take note of it and move on.
...
*and even leaving him a copy of* `"php for dummies-trial.pdf"` *as further insult,*  
...
### Malicious CHM files For The Win
*Searching* **Google**, we come accross this [blog](/assets/Sniper/https://medium.com/r3d-buck3t/weaponize-chm-files-with-powershell-nishang-c98b93f79f1e) explaining exactly **how to execute a payload using a `.chm` file** using a script from the **Nishang script suite.**

*Following the article,* we download the **"HTML Help Workshop and Documentation program"** and **get the script** from the [Github Repository](/assets/Sniper/https://github.com/samratashok/nishang/blob/master/Client/Out-CHM.ps1).
...
We confirmed with `crackmapexec`
