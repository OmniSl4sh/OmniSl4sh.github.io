...
- *Moving along,* we went through the **FTP share** to find a **KeePass database file** which we crack open and gain **creds for an SMB share**.
...
- We **did some research** and were able to **build the exploit binary**. *However,* it only worked when we used the **x86 version** in an **interactive process**.
- We used **Metasploit's multi-handler module** to **catch a standard shell** and **upgrade it to a Meterpreter.**
- And **migrated to a stable interactive process** before running the binary to escalate our privleges to `NT Authority\System` for the 3rd time.
...
3. **MSSQL** is there on default port 1433
4. **WinRM** available
5. Another **unknown HTTP port** there on 15567

## Quick FTP Check
*Before checking SharePoint,* we **pass by FTP** looking for freebies :)
...
This [one](https://github.com/danielmiessler/SecLists/blob/master/Discovery/Web-Content/CMS/sharepoint.txt) from the **SecLists repo** seemed good.
...
*However, to execute commands,* we needed to** enable** the `xp_cmdshell` feature which was **initially unavailable**.
...
Note: *because, over time,* *the* `xp_cmdshell` *option kept getting* ***automatically disabled***, we set the **MSSQL commands** in a file for a bit of automation.
...
We were now inside and interacting as `sarah`
...
That's **ripe for exploitation** using **Juicy Potato**.

we **download the binary** from [Github](https://github.com/ohpe/juicy-potato/releases/tag/v0.1) and call it `jp.exe`

*For me,* that exploit **always worked** when used with a **single executable** rather than **passing arguments**.
...
we can **confirm** that with the PowerShell cmdlet:
...
We then **compile the code** on the victim machine using the **native .NET compiler**
```
...
*To check if we can exploit this,* we run a `Get-ACL` to **find our permissions** on it.
...
The [source code](https://github.com/SecWiki/windows-kernel-exploits/blob/master/CVE-2017-0213/CVE-2017-0213.cpp) was **to be compiled in Visual Studio.**

*without any modifications,* we get a **type error:**

![](/assets/Tally/eop-cve-compile-error.jpg)

*On Google,* we queried with **both the error as well as the variable name** to get **more relevant results**
...
That's because this one has to be run from an "interactive process".
