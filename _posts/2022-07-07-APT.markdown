...
The **Impacket** library had the necessary code and the researcher was kind enough to provide a **python** script for it.
...
we find a password when trying to unzip the file called `backup.zip` which we found.
...
We tried logging in with the Administrator and the Domain Controller machine hashes as well as create a golden ticket with the krbtgt hash since those would be instant kills.
